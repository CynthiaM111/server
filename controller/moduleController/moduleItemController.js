...
const createModuleItem = async (request, response) => {
  ...
  try {
    ...
    if (!updatedModule)
      return response.status(404).json({ error: 'module not found NNNN111' })
    ...
    console.log(err)
    response.status(400).json({ error: err.message +'NNNNN2222'|| err.toString() })
  }
}
...
const updateModuleItem = async (request, response) => {
  ...
  try {
    ...
    if (!updatedModuleItem)
      return response.status(404).json({ error: 'module item not found NNNNN' })
