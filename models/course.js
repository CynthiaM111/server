...
const courseSchema = new mongoose.Schema({
  ...
  university: {
    type: String,
    required: true,    
    enum: ['UR-BUSOGO', 'UR-GIKONDO', 'UR-REMERA','UR-RUSIZI','UR-HUYE',
    'UR-NYARUGENGE','UR-RUKARA','AUKA','UK','AUKA','INES','UNILAK','ULK','KEPLER'],
    default: 'UR-GIKONDO'
...
courseSchema.statics.getCoursesWithPrivilege = async function (userId,userUniversity) {
  const courses = await this.find({university: userUniversity}).populate(
  ...
  const result = courses.map((course) => {
    ...
    if (coursePOJO.enrollments)
      enrollment = coursePOJO.enrollments.find((e) => e.user && e.user._id.equals(userId))
